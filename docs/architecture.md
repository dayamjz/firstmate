...
An expected terminal-ish wait - a crew that reported `done:` with `bin/fm-pr-check.sh`'s merge poll armed, or one that stopped on its `blocked:` stop-and-wait contract - surfaces its idle pane once per wait, is then absorbed even as idle-screen redraws churn the pane hash, re-surfaces only on the bounded pause cadence, and surfaces once again as soon as fresh status activity changes the wait, so neither state can storm per poll.
