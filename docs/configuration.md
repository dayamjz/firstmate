...
FM_BACKEND_HERDR_BARE_PROMPT_RE='^(❯|›)'  # herdr-only: verified agent glyphs recognized as an UNBORDERED (bare) composer row, e.g. claude's ❯ or codex's ›; shell glyphs remain unknown rather than empty, and de-emphasised ghost/placeholder text (dim or dark-truecolor) after an agent prompt reads empty via the shared fm_composer_strip_ghost; written as a glyph alternation, not a bracket expression, so a non-UTF-8 locale cannot byte-match a box corner's shared 0xe2 lead byte (docs/herdr-backend.md "Incident (2026-07-08)", "Incident (2026-07-10)", "Incident (2026-07-26)")
